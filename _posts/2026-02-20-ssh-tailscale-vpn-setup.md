...
현재 구성은 홈 Tailnet 레이어와 외부 VM 접근 레이어로 나뉜다.
Oracle VM은 허용된 출발지 IP만 받기 때문에, `supermini`를 SSH 점프 호스트로 사용해 접속하고 있다.

```mermaid
flowchart TB
  subgraph L1["Layer 1 - Home Tailnet (Tailscale Overlay)"]
    MB["macbookpro\n(Tailscale client)"]
    HS["homeserver\n(SSH server)"]
    SM["supermini\n(Tailscale client + SSH jump host)"]
  end

  CTRL["Tailscale Control Plane\n(Auth / Key Exchange)"]

  MB --- CTRL
  HS --- CTRL
  SM --- CTRL

  MB <-->|SSH via 100.x.x.x| HS
  MB <-->|SSH via 100.x.x.x| SM
  SM <-->|SSH via 100.x.x.x| HS

  subgraph L2["Layer 2 - Oracle Cloud VM (Allowlisted IP only)"]
    OVM["Oracle VM\n(only allowed source IP)"]
  end

  SM -->|"SSH Jump (ProxyJump)\nAllowed source IP = supermini"| OVM
  MB -.->|"Direct access blocked\n(not allowlisted)"| OVM
```
